        ...
        <Box sx={breadcrumbs.box}>
            <MUIBreadcrumbs aria-label='breadcrumbs' separator='·'>
                ...
                {props.subcategory && !projectType
                  ? <Typography sx={breadcrumbs.active}>{props.subcategory}</Typography> : (null)}
                {props.subcategory && projectType ? <Link style={breadcrumbs.link}
                                                          href={{ pathname: '/find-project/subcategories' }}>{props.subcategory}</Link> : (null)}
